		...
		<header className="sticky top-0 z-50 bg-white/70 backdrop-blur border-b border-slate-200">
			<nav className="mx-auto max-w-5xl px-3 sm:px-4 lg:px-6 py-2">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
					<div className="inline-flex items-center h-10 sm:h-12 px-3 border border-slate-200 rounded-lg shadow-2xl bg-white/80">
						<Link to="/" className="inline-flex items-center gap-2 sm:gap-3 select-none min-w-0">
							<img
								src={logoSrc}
								alt={logoAlt}
								className="h-7 w-7 sm:h-8 sm:w-8 object-contain border border-slate-300 rounded-md shrink-0"
								loading="lazy"
							/>
							<span className="truncate text-sm sm:text-base text-slate-800 font-semibold tracking-tight">
								MathLOX{title}
							</span>
						</Link>
					</div>
